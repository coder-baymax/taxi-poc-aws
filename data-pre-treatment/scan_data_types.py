...
def scan_data_types():
    ...
    file_names = s3.list_objects(Bucket=bucket, Prefix=prefix)["Contents"]
    ...
    def get_file_header(file_name):
        for header in s3.get_object(Bucket=bucket, Key=file_name)["Body"].iter_lines():
            header = "".join(re.findall("[a-zA-Z,]+", header.decode("utf8"))).lower()
            return {"name": file_name.split("/")[-1], "header": header}

    with ThreadPoolExecutor(max_workers=32) as executor:
        file_list = list(executor.map(get_file_header, file_names))

    with open("resource/file_headers.txt", "w") as f:
        file_headers = sorted(list(set(x["header"] for x in file_list)))
        f.writelines(f"{x}\n" for x in file_headers)
        f.write("---------------------\n")
        file_list = [(re.findall(r"\d{4}-\d{2}", x["name"])[-1],
